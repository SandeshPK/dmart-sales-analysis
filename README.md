## Dmart Sales Analysis

### Main Dataset (Original)
| Metric         | Value                          |
| -------------- | ------------------------------ |
| Total Records  | **15,345**                     |
| Total Columns  | **12**                         |
| Missing Values | **0**                          |
| Date Range     | **01-Jan-2023 to 31-Dec-2024** |

### Columns available
| Column        | Data Type | Description             |
| ------------- | --------- | ----------------------- |
| order_id      | Text      | Unique order identifier |
| region        | Text      | Sales region            |
| category      | Text      | Product category        |
| sales         | Numeric   | Sales amount            |
| discount      | Numeric   | Discount applied        |
| quantity      | Integer   | Quantity sold           |
| order_date    | Date      | Order date              |
| delivery_date | Date      | Delivery date           |
| customer_type | Text      | New / Regular / Member  |
| payment_mode  | Text      | Cash / Card / UPI       |
| profit        | Numeric   | Profit earned           |
| city          | Text      | Customer city           |

### Overall Business mertrics
| Metric              |             Value |
| ------------------- | ----------------: |
| Total Sales         | **39,262,877.52** |
| Total Profit        |  **6,872,139.81** |
| Total Quantity Sold |        **76,831** |

### Region-wise Record Count
| Region | Records |
| ------ | ------: |
| North  |   3,924 |
| South  |   3,853 |
| West   |   3,790 |
| East   |   3,778 |

### Category-wise Record Count
| Category      | Records |
| ------------- | ------: |
| Electronics   |   3,115 |
| Home Care     |   3,115 |
| Personal Care |   3,088 |
| Grocery       |   3,025 |
| Clothing      |   3,002 |

### City-wise Record Count
| City      | Records |
| --------- | ------: |
| Bangalore |   2,606 |
| Chennai   |   2,565 |
| Delhi     |   2,559 |
| Mumbai    |   2,543 |
| Pune      |   2,539 |
| Hyderabad |   2,533 |

### Payment Mode Distribution
| Payment Mode | Records |
| ------------ | ------: |
| UPI          |   5,136 |
| Cash         |   5,109 |
| Card         |   5,100 |

### Customer Type Distribution
| Customer Type | Records |
| ------------- | ------: |
| New           |   5,156 |
| Regular       |   5,144 |
| Member        |   5,045 |
